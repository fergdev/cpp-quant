...
static asio::awaitable<void> md_loop(MdBinance* self) {
  try {
    ...
    auto r = co_await res.async_resolve(self->host, self->port, asio::use_awaitable);

    beast::ssl_stream<beast::tcp_stream> tls{self->ioc, self->ssl_ctx};
    co_await beast::get_lowest_layer(tls).async_connect(*r.begin(), asio::use_awaitable);
    ...
    beast::websocket::stream<beast::ssl_stream<beast::tcp_stream>> ws{std::move(tls)};
    ...
    for (;;) {
      ...
      Tick t{
        ...
        std::stod(j.value("c","0")), std::stod(j.value("b","0")),
        std::stod(j.value("a","0")), std::stod(j.value("v","0"))
      };
...
MdBinance::MdBinance(asio::io_context& io, ssl::context& ssl, Channel<Tick,8192>& out)
  : ioc(io), ssl_ctx(ssl), out(out) {}

void MdBinance::start() {
  asio::co_spawn(ioc, md_loop(this), asio::detached);
}
